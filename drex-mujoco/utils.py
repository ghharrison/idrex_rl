...
def gen_traj_with_gaussian_noise(env,agent,min_length, epsilon):
    obs, actions, rewards = [env.reset()], [], []
    while True:
        action = agent.act(obs[-1], None, None)
        ob, reward, done, info = env.step(action)
        ...
        # print("Action: ", action)
        # print("Reward: ", reward)
        # Add random noise according to epsilon
        noise = epsilon * np.random.normal(np.zeros(3), 1)
...
class RewardNet():
    ...
    def build_input_placeholder(self,name):
        return tf.compat.v1.placeholder(tf.float32,[None,self.in_dims],name=name)
...
class Model(object):
    def __init__(self,net:RewardNet,batch_size=64):
        ...
        self.x_split = tf.compat.v1.placeholder(tf.int32,[self.B]) # B-lengthed vector indicating the size of each steps
        self.y = net.build_input_placeholder('y') # tensor shape of [B*steps] + input_dims
        self.y_split = tf.compat.v1.placeholder(tf.int32,[self.B]) # B-lengthed vector indicating the size of each steps
        self.l = tf.compat.v1.placeholder(tf.int32,[self.B]) # [0 when x is better 1 when y is better]
    ...
    def parameters(self,train=False):
        if train:
            return tf.compat.v1.trainable_variables(self.net.param_scope.name)
        else:
            return tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES,self.net.param_scope.name)
    ...
    def train(self,D,iter=10000,l2_reg=0.01,noise_level=0.1,debug=False,early_term=False):
        ...
        def load(idxes):
            b_x,b_y,x_split,y_split,b_l =\
                tf.compat.v1.py_func(_load, [idxes], [tf.float32,tf.float32,tf.int64,tf.int64,tf.int64], stateful=False)
        ...
        batch_op = tf.compat.v1.data.make_one_shot_iterator(ds).get_next()
